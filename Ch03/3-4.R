...
#select - DF의 특정 컬럼을 선택해서 새로운 DF생성
...
df_result <- df_exam %>% filter(math >= 60) %>% 
                         ...
                         arrange(desc(math)) %>% 
                         head(3)
df_result #filter 랑 select의 자리 바꿔도 무방(일반 쿼리문과 거의 동일)

#mutate - DF의 새로운 특정 컬럼(파생변수수)을 추가해서 새로운 DF생성
...
df_grade <- df_mean %>% mutate(grade = ifelse(df_mean$mean >= 90, 'A', 
                                       ifelse(df_mean$mean >= 80, 'B', 
                                       ifelse(df_mean$mean >= 70, 'C', 
                                       ifelse(df_mean$mean >= 60, 'D', 'F')))))
df_grade
...
#inner_join - 두 개 이상의 DF의 공통 컬럼으로 조인
